...
`98 passed`, todo lo que no depende de la red está funcionando.
...
| 8. Ejecución en paper trading | Prueba de fase 0 corrida en paper (14/09/2026): crédito con límite negativo; la conversión fue rechazada por enviar la `mleg` con acciones parciales. Implementado y testeado con broker falso: secuencia de paridad con fill completo, ejecución `mleg`, ledger SQLite con reconciliación, loop `paper-run` (dry-run por defecto) y reporte de sesión. Falta correrlo contra la API real. |
